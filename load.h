/* Load the entire input from in into a memory buffer. The allocated memory
   ...
   multiple load() calls. size can be NULL, in which case the size of the
   allocation is not returned, and reuse is not supported.
   ...
   allocation of size *size, unless size is NULL.
 */
